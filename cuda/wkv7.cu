    ...
    F *__restrict__ const _gw)
{
    ...
    float sa[_N_] = {0};
    float sb[_N_] = {0};
    __shared__ float r[_N_], k[_N_], w[_N_], a[_N_], b[_N_], gy[_N_];
    ...
    for (int _t = T - 1; _t >= 0; _t--)
    {
        ...
        for (int j = 0; j < _N_; j++)
        {
            ...
            float& sa_ = sa[j];
            float& sb_ = sb[j];
            s = s * w[j] + k[j] * v[_t] + sa_ * b[j];
            sb_ = sb_ * w[j] + a[j] * gy[j];
            sss += s * gy[j];
            sa_ = sa_ * w[j] + a[j] * s;
        }
...
void cuda_backward(int B, int T, int C, int H, bf16 *r, bf16 *w, bf16 *k, bf16 *v, bf16 *a, bf16 *b, bf16 *gy, bf16 *gr, bf16 *gk, bf16 *gv, bf16 *gw, bf16 *ga, bf16 *gb)
{
    assert(H*_N_ == C);
    kernel_backward<<<dim3(B * H), dim3(_N_)>>>(B, T, C, H, r, w, k, v, u, gy, gr, ga, gb);
}